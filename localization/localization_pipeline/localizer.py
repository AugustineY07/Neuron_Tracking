...
class LOCALIZER(object):
    ...
    def __init__(self, bin_file, dtype, spike_index_path, templates_path, geom_path, denoiser_weights, offset_detector_denoiser, 
                 ...
                 multi_processing = 0, n_processors = 1, spike_size = 121, n_channels_loc = 10):
    #def __init__(self, bin_file, dtype, spike_index_path, templates_path, geom_path, denoiser_weights, offset_detector_denoiser, 
                 #n_filters, filter_sizes, sampling_rate, spike_size, n_channels_loc,
                 #multi_processing = 0, n_processors = 1):
        self.bin_file = bin_file
        self.dtype = np.dtype(dtype)
        self.spike_index = np.load(spike_index_path) #input spike and max channel
    ...
    def read_waveforms(self, spike_times, n_times=None, channels=None):
        ...
        if n_times is None:
            n_times = self.spike_size #121 or 81 in KS
        ...
        wfs = np.zeros((len(spike_times), n_times, len(channels)),
                       'float32') #store waveforms, dimension = num_spikes * 121 * 95

        skipped_idx = [] #why need to skip 
        total_size = n_times*self.n_channels
        # spike_times are the centers of waveforms
        #spike_times_shifted = spike_times - 42;
        spike_times_shifted = spike_times - n_times//2 #move spike times by half of length: 42-60, try moving it to 42 
...
#         torch.cuda.set_device(CONFIG.resources.gpu_id)
    ...
    def get_estimate(self, batch_id, threshold = 4, output_directory = 'position_results_files'):
        ...
        # understand how is batch determined 
...
#                 mc = wfs_0.ptp(0).argmax()
        ...
        #fname_ids = os.path.join(output_directory, 'ids_{}.npy'.format(str(batch_id).zfill(6)))
        #fname_spikes = os.path.join(output_directory, 'spike_times_batch_{}.npy'.format(str(batch_id).zfill(6)))
        ...
        #np.save(fname_ids, skipped_idx)
        #np.save(fname_spikes, spike_times_batch)
