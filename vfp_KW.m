...
input_name = [input_struct.subject,'_PSTH.mat'];
...
for ish = 1:shank %loop on shanks  
    for id = 1:day-1 % loop on days
        ...
        shank(:,6) = [1:1:length(good_clu(id,ish).clu(p_sig{id,ish}))]'; %order in rsp and PSTH by physical location
...
% calculate rsp and PSTH
count = 0;
for ish = 1:size(rsp,2) %loop on shanks  
    for id = 1:size(rsp,1)-1 % loop on days
        % calculate correlation of rsp spike count and mean PSTH between the same shank different day
%         rsp_sort = rsp_sig{id,ish}(shank(:,6),:); %rearrange based on physical location
%         PSTH_sort = PSTH_sig{id,ish}(shank(:,6),:);
%         rsp_sort2 = rsp_sig{id+1,ish}(shank2(:,6),:); %rearrange based on physical location
%         PSTH_sort2 = PSTH_sig{id+1,ish}(shank2(:,6),:);
...
save(fullfile(output_path,output_name), 'good_clu_KW', 'p_sig','clu_sig', 'rsp_sig', 'sim_rsp_sig', 'PSTH_sig', 'sim_PSTH_sig','simScore_sig')
